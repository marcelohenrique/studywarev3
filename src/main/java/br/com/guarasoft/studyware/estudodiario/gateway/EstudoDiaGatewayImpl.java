...
@Stateless
public class EstudoDiaGatewayImpl extends AbstractDao<EstudoDiarioTransformer, Long> implements EstudoDiaGateway {
	...
	public List<EstudoSemanal> findAll(Long idEstudo) {
		...
		sql.append(
				"SELECT new br.com.guarasoft.studyware.estudodiario.gateway.entidade.EstudoDiarioTransformer( date_trunc( 'day', emh.horaEstudo ), extract( DOW FROM emh.horaEstudo ), ed.tempoAlocado, SUM( emh.tempoEstudado ) ) ");
		...
		sql.append(
				" GROUP BY date_trunc( 'day', emh.horaEstudo ), extract( DOW FROM emh.horaEstudo ), ed.tempoAlocado ");
		sql.append(" ORDER BY date_trunc( 'day', emh.horaEstudo ) ");

		TypedQuery<EstudoDiarioTransformer> query = this.entityManager.createQuery(sql.toString(),
				EstudoDiarioTransformer.class);
		query.setParameter("idEstudo", idEstudo);
		List<EstudoDiarioTransformer> entidades = query.getResultList();

		List<EstudoSemanal> beans = this.estudoDiarioEntidadeConverter.convert(entidades);
